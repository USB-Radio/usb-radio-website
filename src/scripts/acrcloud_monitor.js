...
// Metadata para exportar
export let currentMode = "N.A";

//----------------------------------------------------------------------
const sendBufferToAcrCloud = async (buffer) => {
  console.log(
    chalk.yellow("\n* Preparando el buffer para enviar a ACRCloud...")
  );
  ...
  const signature = crypto
    .createHmac("sha1", acrCloudConfig.access_secret)
  ...
  try {
    ...
    console.log(chalk.green("\n* Respuesta de ACRCloud recibida. "));
    return response.data;
  } catch (error) {
    console.error(chalk.red("Error al enviar a ACRCloud:", error));
    throw error;
  }
};
...
const startGoogleSpeechStream = () => {
  // console.log(
  //   "Iniciando transcripción en tiempo real con Google Speech-to-Text..."
  // );
  ...
  const createRecognizeStream = () => {
    const recognizeStream = client
      ...
      .on("error", (error) => {
        console.error(
          chalk.red("Error en el streaming de Google Speech-to-Text:", error)
        );
        if (error.code === 11) {
          ...
          if (isFinal || currentTime - lastWriteTime >= 5000) {
            Object.keys(speakerTextMap).forEach((speakerTag) => {
              ...
              const formatTime = (time) => {
                const seconds =
                  parseInt(time.seconds || 0, 10) +
                  parseFloat(time.nanos || 0) / 1e9;
                const date = new Date(seconds * 1000);
                const hours = String(date.getUTCHours()).padStart(2, "0");
                const minutes = String(date.getUTCMinutes()).padStart(2, "0");
                const secs = String(date.getUTCSeconds()).padStart(2, "0");
                const millis = String(
                  Math.floor(date.getUTCMilliseconds())
                ).padStart(3, "0");
              ...
              // AQUI SE ENCUENTRAN LOS DATOS DE TEXTO GENERADOS PARA EXPORTAR.
              // datos de tipo string
              const speechSubs = {
                subtitles: speakerData.words.join(" "),
              };
              // console.log("--> ", speechSubs);
              axios.post(`${API}/speech`, speechSubs);
...
const startMonitoring = async () => {
  currentMode = "song"; // Inicialmente asumimos que está en modo canción
  ...
  const ffmpegProcess = ffmpeg(streamUrl)
    .format("mp3")
    .on("error", (error) => {
      console.error(chalk.red("Error al capturar audio:", error));
    });
  ...
  while (true) {
    try {
      ...
      if (currentTime - lastCheckTime >= checkInterval * 1000) {
        ...
        console.log(
          chalk.blue("\n* Verificando si hay una canción en reproducción...")
        );
        ...
        if (acrData.status.code === 0 && acrData.status.msg === "Success") {
          let song = acrData.metadata.music[0];
          // console.log(
          ...
          // console.log(
          //   "Metadata completa de la canción:",
          //   JSON.stringify(song, null, 2)
          // );

          // Enviar petición post a API
          ...
          console.log(
            chalk.red(
              "\n* No se detectó canción, continuando en modo podcast..."
            )
          );
